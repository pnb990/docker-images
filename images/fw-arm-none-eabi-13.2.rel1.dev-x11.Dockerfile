...
#
# The whole feature is 11 MB on top of `python`. What it deliberately leaves
# out is Qt: PySide6/PyQt need libGL and libEGL, and the mesa stack behind
# them is 75 MB before a single Qt library is installed -- 118 MB with
# Debian's python3-pyqt6. Nothing in the projects uses Qt today. If that
# changes, the wheels from PyPI carry Qt itself and only need the system
# libraries here:
#
#     libgl1 libegl1 libxkbcommon-x11-0 libdbus-1-3 fontconfig \
