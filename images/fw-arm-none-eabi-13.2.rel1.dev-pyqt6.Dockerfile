...
# variant  : dev-pyqt6
# features : locale -> build -> python -> uv -> arm-13.2.rel1 -> ci-runtime -> dev -> new_git -> clang -> jlink -> x11 -> pyqt6
...
RUN printf 'IMAGE_NAME="%s"\nIMAGE_VARIANT="%s"\nIMAGE_COMMIT="%s"\nIMAGE_DATE="%s"\n' \
        'fw-arm-none-eabi-13.2.rel1' 'dev-pyqt6' "$IMAGE_COMMIT" "$IMAGE_DATE" \
        > /etc/image-info
USER ${USERNAME}
